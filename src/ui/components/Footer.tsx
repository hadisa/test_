	...
	return (
		<footer className="border-neutral-300 dark:bg-bgDark bg-neutral-50">
			<div className="mx-auto max-w-7xl px-4 lg:px-8">
							...
							<div key={item.id}>
								...
								<ul className="mt-4 space-y-4 [&>li]:text-neutral-500">
									{item.children?.map((child) => {
										if (child.category) {
											return (
												<li key={child.id} className="text-sm text-gray-500 dark:text-gray-200">
				...
				{channels?.channels && (
					<div className="mb-4 ext-neutral-500 dark:text-gray-300">
						<label>
							<span className="text-sm text-gray-500 dark:text-gray-200">Change currency:</span> <ChannelSelect channels={channels.channels} />
						</label>
					</div>
				)}
